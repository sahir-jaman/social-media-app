    ...
    def post(self, request, formate =None):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if exists and otp!=None:
                ...
                if is_valid:
                    token = get_tokens_for_user(user)
                    return Response({'Token': token, "success": "User Logged in Successfully"}, status= status.HTTP_201_CREATED)
                else:
                    return Response({"status": "Email or OTP not matched"}, status= status.HTTP_400_BAD_REQUEST)

            else:
                return Response({"Error":"Email or OTP not matched"}, status= status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request, format =None):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if exists:
                ...
                model_instance.save()
                return HttpResponse(f"Auto-generated OTP: {new_otp}", status= status.HTTP_201_CREATED)
            else:
                return Response({"Error":"There is no existing user found for this email"}, status= status.HTTP_400_BAD_REQUEST)
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
