...
class PostDetail(APIView):
    ...
    def put(self, request, uid, format=None):
        ...
        if serializer.is_valid():
            ...
            return Response({"message": "The post updated successfully"}, status= status.HTTP_201_CREATED)
        else:
            return Response(
                {"message": "Error updating post", "errors": serializer.errors}, status= status.HTTP_400_BAD_REQUEST
            )
    ...
    def delete(self, request, uid, format=None):
        try:
            ...
            if post.user.id == request.user.id:
                ...
                return Response({"success": False, "message": "not enough permissions"}, status= status.HTTP_400_BAD_REQUEST)
        except Post.DoesNotExist:
...
class CommentPost(APIView):
    ...
    def get(self, request, uid, format=None):
        try:
            ...
            return Response({"success": True, "comments": serializer.data}, status= status.HTTP_201_CREATED)
        except Post.DoesNotExist:
            return Response({"success": False, "message": "post does not exist"}, status= status.HTTP_400_BAD_REQUEST)
